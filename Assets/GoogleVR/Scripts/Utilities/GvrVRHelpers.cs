...
public static class GvrVRHelpers
{
    ...
    public static Quaternion GetHeadRotation()
    {
        ...
        if (headDevice.TryGetFeatureValue(CommonUsages.deviceRotation, out rotation))
        {
            return rotation;
        }
        else
        {
            Debug.LogWarning("Unable to retrieve head rotation.");
            return Quaternion.identity;
        }
#endif // UNITY_EDITOR
    }
    ...
    /// <remarks>
    /// This is the point at which `Hybrid` mode will transition from `Direct` (closer than the
    ...
    /// <returns>Returns `true` if the laser is shrunken, `false` otherwise.</returns>
